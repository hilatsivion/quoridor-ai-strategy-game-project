...
class Pawn(Drawable):
    ...
    def __init__(self,
        ...
        self.power_bombs = 1  # Each player gets one power bomb
        self.__cell = None
        self.id = core.PAWNS  # FIXED: Set ID before calling set_goal()
        self.set_goal()
    ...
    def set_goal(self):
        ...
        player.
        """
        # FIXED: Goals based on player color (more reliable than ID)
        # PAWN_A_COL (red/human): starts at bottom (row 8), goal is top (row 0)
        # PAWN_B_COL (blue/AI): starts at top (row 0), goal is bottom (row 8)
        import config as cfg
        if self.color == cfg.PAWN_A_COL:  # Human player (red)
    ...
    @property
    def state(self):
        """ Returns a string containing i,j,w,p being i, j the pawn coordinates,
        w the number of remaining walls, and p the number of power bombs
        """
        return '%i%i%02i%i' % (self._coord.row, self._coord.col, self.walls, self.power_bombs)
