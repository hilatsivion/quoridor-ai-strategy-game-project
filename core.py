...
class DistArray(CellArray):
    ...
    def update_cell(self, coord: Coord):
        ...
        # Safety check: Handle case where no valid moves exist (blocked position)
        if not values:
            # If completely blocked, distance is infinite
            newval = cfg.INF
        else:
            newval = 1 + min(values)
    ...
    def shortest_path_len(self):
        """ Return len of the shortest path from current position
        """
        if self.pawn.coord in self.pawn.goals:
            return 0  # Already at goal
        
        valid_moves = self.pawn.valid_moves
        if not valid_moves:  # SAFETY: Handle corrupted board state
            # This should never happen in valid Quoridor, but protects against simulation bugs
            return cfg.INF  # Infinite distance if completely blocked
        
        # Distance from current position = 1 + minimum distance from next moves
        return 1 + min([self.get_cell(pos) for pos in valid_moves])
